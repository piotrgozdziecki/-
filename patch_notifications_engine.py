...
function showAnnouncement(txt, col = '#38bdf8') {
  const now = Date.now();
  if (txt === lastAnnouncementText && (now - lastAnnouncementTime < 4000)) return; // Debounce
...
// --- QUEUED ACHIEVEMENT TOAST ENGINE ---
...
if ann_func_old in text:
    ...
    print("Replaced announcement and achievement functions.")
...
function addSpeechBubble(x, y, text, color = '#ffffff') {
  ...
  const now = Date.now();
  if (isPlayer) {
    if (now - lastPlayerSpeechTime < 2800) return; // Throttle player speech to 2.8s
...
if speech_func_old in text:
    ...
    print("Throttled speech bubbles.")
...
if kill_combo_old in text:
    ...
    print("Replaced kill combo milestone logic.")

with open('app/src/main/assets/game.html', 'w', encoding='utf-8') as f:
    f.write(text)

print("Updated game.html notification engine.")
