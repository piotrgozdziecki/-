...
const cinemaPipeline = {
  ...
  renderScreenSpaceCinematic(ctx) {
    ...
    if (screenShake > 4) {
      ctx.save();
      ctx.globalCompositeOperation = 'screen';
      ctx.fillStyle = `rgba(239, 68, 68, ${Math.min(0.2, screenShake * 0.02)})`;
      ctx.fillRect(2, 0, gameWidth, gameHeight);
      ctx.fillStyle = `rgba(56, 189, 248, ${Math.min(0.2, screenShake * 0.02)})`;
      ctx.fillRect(-2, 0, gameWidth, gameHeight);
      ctx.restore();
    }
...
let screenShake = 0; // Disabled per user request

// Crimsonland Blood & Epoxy Stains
const bloodStains = [];
function addBloodStain(x, y, enemyInfo) {
  if (window.Engine3D && window.Engine3D.active && window.Engine3D.spawnBlood) {
    window.Engine3D.spawnBlood(x, y, enemyInfo);
  }
...
function createSparks(x, y, count, color = '#f59e0b') {
  if (window.Engine3D && window.Engine3D.active && window.Engine3D.spawnSparks) {
    let colHex = 0xf59e0b;
    if (color === '#ef4444') colHex = 0xef4444;
    else if (color === '#38bdf8') colHex = 0x38bdf8;
    else if (color === '#4ade80') colHex = 0x4ade80;
    else if (color === '#ec4899') colHex = 0xec4899;
    window.Engine3D.spawnSparks(x, y, 14, Math.min(count * 2, 24), colHex);
  }
...
function spawnBatchByTime(currentMinutes) {
  ...
  // MUCH HIGHER ENEMY DENSITY (CRIMSONLAND STYLE)
  const progressInSector = Math.min(1.0, (gameTime % 120) / 120);
  const baseBatchSize = Math.floor(12 + progressInSector * 20.0 + Math.random() * 8.0);
...
function checkLevelUp() {
  ...
  while (currentXP >= neededXP) {
    currentXP -= neededXP;
    playerLevel++;
    // Balanced hardcore progression: higher growth rate so perks feel earned and rare
    neededXP = Math.floor(neededXP * 1.68 + 80);
    levelUpPendingCount++;
    leveledUp = true;
  }
...
function triggerLevelUpModal(isReroll = false) {
  ...
  Object.keys(EVOLUTIONS).forEach(evoKey => {
    ...
    if (w && w.level >= 5 && p && p.level >= 1 && !w.isEvo) {
      options.push({ cat: 'evolution', evo: evo });
    }
  });
...
function fireStapler() { spawnMuzzleFlash(player.x + Math.cos(player.angle)*20, player.y + Math.sin(player.angle)*20, 35, '#fef08a'); spawnBarrelSmoke(player.x, player.y, player.angle);  ejectSpentShell(player.x, player.y, player.angle); if (sounds && sounds.heavyGunshot) sounds.heavyGunshot();
  if (enemies.length === 0) return;
  sounds.hit();
...
function fireScanner() { spawnMuzzleFlash(player.x + Math.cos(player.angle)*15, player.y + Math.sin(player.angle)*15, 45, '#38bdf8');  if (Math.random() < 0.4) ejectSpentShell(player.x, player.y, player.angle);
...
function firePallet() { spawnMuzzleFlash(player.x + Math.cos(player.angle)*24, player.y + Math.sin(player.angle)*24, 50, '#f59e0b'); spawnBarrelSmoke(player.x, player.y, player.angle);  ejectSpentShell(player.x, player.y, player.angle);
  if (enemies.length === 0) return;
  let target = null;
...
function damageEnemy(e, amount) {
  ...
  if (isCrit || e.isBoss) {
      ...
      if (window.AndroidBridge && window.AndroidBridge.vibrate) {
        window.AndroidBridge.vibrate(e.isBoss ? 50 : 25);
      }
  }
...
function drawAtmosphericDust(ctx, viewW, viewH) {
  for (let i = 0; i < dustMotes.length; i++) {
    const dm = dustMotes[i];
    if (dm.x > camera.x && dm.x < camera.x + viewW && dm.y > camera.y && dm.y < camera.y + viewH) {
      const dx = dm.x - player.x;
      const dy = dm.y - player.y;
      const dist = Math.hypot(dx, dy);
      let intensity = dm.alpha * 0.4;
      if (dist < 320) {
        let angleToMote = Math.atan2(dy, dx);
        let diff = Math.abs(angleToMote - player.angle);
        while (diff > Math.PI) diff = Math.abs(diff - Math.PI * 2);
        if (diff < 0.6) {
          intensity = 0.85 * (1 - dist / 320);
        }
      }
...
function drawTacticalHUDOverlay(ctx) {
  ...
  ctx.textAlign = 'left';
  const posText = `SECTOR 8F // POS: ${Math.round(player.x)},${Math.round(player.y)} // STATUS: COMBAT_ACTIVE // THREATS: ${enemies.length}`;
...
if (e.isBoss) {
    // Boss guarantees one treasure chest
...
function update(dt) {
  ...
  let canForklift = (player.hasForklift && player.wantsForklift && player.stamina > 0 && !player.staminaLock);
  ...
  for (let i = 0; i < obstacles.length; i++) {
    ...
    if (distance < player.radius) {
      ...
      if (distance > 0.01) {
        ...
        if (pdist > 0.01) {
          player.x += (pdx / pdist) * overlap;
          player.y += (pdy / pdist) * overlap;
        } else {
          player.x -= player.vx * dt;
          player.y -= player.vy * dt;
        }
  ...
  for (let i = projectiles.length - 1; i >= 0; i--) {
    ...
    if (p.type === 'pallet') {
      ...
      for (let j = 0; j < enemies.length; j++) {
        ...
        if (Math.hypot(e.x - p.x, e.y - p.y) < e.info.radius + 15) {
          damageEnemy(e, p.damage);
          p.hitList.push(e.id);
...
function draw25DPlayerForklift(ctx) {
  ...
  if (player.isForklift) {
    // 1. Directional 2.5D Ground Shadow (Elongated along warehouse light angle)
    ctx.fillStyle = 'rgba(0,0,0,0.52)';
    ctx.beginPath();
    ctx.ellipse(6, 12, 34, 22, player.angle + 0.12, 0, Math.PI * 2);
    ctx.fill();

    ctx.rotate(player.angle);

    // 2. Heavy Industrial Solid Rubber Wheels (4x with steel rims & chrome hubs)
    const wheels = [
      { x: -21, y: -19 }, // Rear left
      { x: -21, y: 13 },  // Rear right
      { x: 14, y: -19 },  // Front left
      { x: 14, y: 13 }    // Front right
    ];
    for (let w = 0; w < wheels.length; w++) {
      const wh = wheels[w];
      // Dark tire tread
      ctx.fillStyle = '#090d16';
      ctx.fillRect(wh.x, wh.y, 11, 6);
      ctx.strokeStyle = '#1e293b';
      ctx.lineWidth = 0.8;
      ctx.strokeRect(wh.x, wh.y, 11, 6);
      // Steel rim & hub nut
      ctx.fillStyle = '#64748b';
      ctx.fillRect(wh.x + 3, wh.y + 1, 5, 4);
      ctx.fillStyle = '#e2e8f0';
      ctx.fillRect(wh.x + 4, wh.y + 2, 3, 2);
    }

    // 3. Massive 3D Cast Iron Rear Counterweight (Chamfered Bevels & Tow Pin)
    const cwGrad = ctx.createLinearGradient(-30, 0, -18, 0);
    cwGrad.addColorStop(0, '#090d16');
    cwGrad.addColorStop(0.5, '#1e293b');
    cwGrad.addColorStop(1, '#334155');
    ctx.fillStyle = cwGrad;
    ctx.fillRect(-29, -15, 11, 30);
    // Bevelled top face highlight
    ctx.fillStyle = '#475569';
    ctx.fillRect(-29, -15, 4, 30);
    // Tow pin socket
    ctx.fillStyle = '#dc2626';
    ctx.fillRect(-30, -3, 3, 6);

    // 4. Toyota Industrial Safety Orange Chassis (3D Dual-Tone Shading)
    const chassisGrad = ctx.createLinearGradient(-25, -16, 25, 16);
    chassisGrad.addColorStop(0, '#c2410c'); // Deep lower skirt shadow
    chassisGrad.addColorStop(0.3, '#ea580c');
    chassisGrad.addColorStop(0.7, '#f97316'); // Vibrant Toyota Orange
    chassisGrad.addColorStop(1, '#ea580c');
    ctx.fillStyle = chassisGrad;
    ctx.fillRect(-25, -16, 49, 32);

    // Chassis Top Deck Plate (Specular Chamfer Edge)
    ctx.strokeStyle = '#fed7aa';
    ctx.lineWidth = 1.6;
    ctx.strokeRect(-25, -16, 49, 32);

    // Black Rubber Side Rub-Rails / Bumper Strips
    ctx.fillStyle = '#090d16';
    ctx.fillRect(-24, -17, 47, 2);
    ctx.fillRect(-24, 15, 47, 2);

    // 5. Operator Cockpit Interior
    // Non-slip rubber floor mat
    ctx.fillStyle = '#030712';
    ctx.fillRect(-14, -13, 20, 26);
    // Contoured ergonomic bucket seat with headrest
    ctx.fillStyle = '#1e293b';
    ctx.beginPath();
    ctx.roundRect(-10, -8, 12, 16, 4);
    ctx.fill();
    ctx.fillStyle = '#334155';
    ctx.fillRect(-12, -5, 3, 10); // Headrest
    // Steering column with tilt wheel & spinner knob
    ctx.fillStyle = '#64748b';
    ctx.beginPath(); ctx.arc(3, -2, 4, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#cbd5e1';
    ctx.beginPath(); ctx.arc(5, -3, 1.8, 0, Math.PI * 2); ctx.fill(); // Spinner knob
    // Glowing green battery telemetry LED
    ctx.fillStyle = '#22c55e';
    ctx.fillRect(4, 3, 2, 2);
    ctx.fillStyle = '#38bdf8';
    ctx.fillRect(4, 6, 2, 2);

    // 6. 3D Tubular Steel Safety Roll Cage (ROPS / FOPS Overhead Grille)
    ctx.strokeStyle = '#020617';
    ctx.lineWidth = 2.8;
    ctx.strokeRect(-15, -13, 21, 26);
    // Overhead steel mesh & tinted sky-window
    ctx.fillStyle = 'rgba(15, 23, 42, 0.88)';
    ctx.fillRect(-14, -12, 19, 24);
    ctx.fillStyle = 'rgba(56, 189, 248, 0.42)';
    ctx.fillRect(-12, -10, 15, 20);

    // 7. Heavy-Duty Chrome Hydraulic Mast & Forged Steel Forks
    // Mast vertical I-beam rails
    const mastGrad = ctx.createLinearGradient(20, 0, 26, 0);
    mastGrad.addColorStop(0, '#475569');
    mastGrad.addColorStop(0.5, '#cbd5e1');
    mastGrad.addColorStop(1, '#1e293b');
    ctx.fillStyle = mastGrad;
    ctx.fillRect(20, -15, 6, 30);
    // Central chrome hydraulic ram
    ctx.fillStyle = '#f8fafc';
    ctx.fillRect(21, -4, 4, 8);
    // Heavy dual leaf lifting chains
    ctx.fillStyle = '#0f172a';
    ctx.fillRect(24, -10, 2, 20);

    // Dual Forged Steel Tines (Widły) extending forward at ground level
    const forkGrad = ctx.createLinearGradient(26, 0, 52, 0);
    forkGrad.addColorStop(0, '#64748b');
    forkGrad.addColorStop(0.6, '#cbd5e1');
    forkGrad.addColorStop(1, '#f1f5f9');
    ctx.fillStyle = forkGrad;
    // Left fork tine
    ctx.fillRect(26, -13, 25, 6);
    ctx.strokeStyle = '#94a3b8';
    ctx.lineWidth = 0.8;
    ctx.strokeRect(26, -13, 25, 6);
    // Right fork tine
    ctx.fillStyle = forkGrad;
    ctx.fillRect(26, 7, 25, 6);
    ctx.strokeRect(26, 7, 25, 6);

    // 8. Rotating Amber Safety Strobe Beacon (3D Roof Mounted)
    const strobeRot = gameTime * 8;
    ctx.fillStyle = '#f59e0b';
    ctx.beginPath(); ctx.arc(-5, 0, 6, 0, Math.PI * 2); ctx.fill();
    ctx.fillStyle = '#fef08a';
    ctx.beginPath(); ctx.arc(-5, 0, 3, 0, Math.PI * 2); ctx.fill();
    // Dual rotating light beams sweeping around roof
    ctx.fillStyle = 'rgba(254, 240, 138, 0.4)';
    ctx.beginPath();
    ctx.moveTo(-5, 0);
    ctx.arc(-5, 0, 22, strobeRot, strobeRot + 0.5);
    ctx.closePath();
    ctx.fill();
    ctx.beginPath();
    ctx.moveTo(-5, 0);
    ctx.arc(-5, 0, 22, strobeRot + Math.PI, strobeRot + Math.PI + 0.5);
    ctx.closePath();
    ctx.fill();

    // 9. Exhaust Pipe & Turbo Flame Plume
    ctx.fillStyle = '#334155';
    ctx.fillRect(-30, 6, 5, 6);
    if (packageCombo > 0 || player.isDashing) {
      const flameLen = Math.min(32, 8 + packageCombo * 1.3 + Math.sin(gameTime * 28) * 4);
      const flameGrad = ctx.createLinearGradient(-30, 9, -30 - flameLen, 9);
      if (packageCombo >= 15) {
      ...
      ctx.beginPath();
      ctx.moveTo(-30, 7);
      ctx.lineTo(-30 - flameLen, 9);
      ctx.lineTo(-30, 11);
      ctx.closePath();
      ctx.fill();
    }
  } else {
    // On-foot Pedestrian Player Rendering (3D Walking Silhouette)
    ctx.rotate(player.angle);
    // Ground Shadow
    ctx.fillStyle = 'rgba(0,0,0,0.52)';
    ctx.beginPath();
...
function drawSingleMapProp(ctx, p) {
  ...
  ctx.beginPath();
  ctx.ellipse(2, 10, p.radius * 1.15, p.radius * 0.6, 0, 0, Math.PI * 2);
  ctx.fill();
  if (p.hitFlash > 0) {
    ctx.fillStyle = '#ffffff';
    ctx.beginPath(); ctx.arc(0, 0, p.radius + 2, 0, Math.PI * 2); ctx.fill();
  }
  // 3D Extruded Prop Body
  ctx.fillStyle = p.def.color;
  ctx.beginPath();
  ctx.roundRect(-p.radius, -p.radius, p.radius * 2, p.radius * 2, 6);
  ctx.fill();
  ...
  ctx.lineWidth = 2;
  ctx.stroke();
  // 3D Top Lid / Face Highlight
  ctx.fillStyle = 'rgba(255, 255, 255, 0.22)';
  ctx.beginPath();
  ctx.roundRect(-p.radius + 2, -p.radius + 2, p.radius * 2 - 4, p.radius * 0.7, 4);
  ctx.fill();
  ctx.fillStyle = '#ffffff';
...
function drawSingleDropItem(ctx, it, fontScale) {
  ...
  // Hovering 2.5D Float Bob
  const floatBob = Math.sin(gameTime * 6 + it.x) * 4;
  ctx.translate(0, floatBob);

  // Drop shadow on floor beneath pickup
  ctx.fillStyle = 'rgba(0, 0, 0, 0.38)';
  ctx.beginPath();
  ctx.ellipse(0, -floatBob + 6, 12, 6, 0, 0, Math.PI * 2);
  ctx.fill();
...
function render(dt = 0.016) {
  ...
  const sec = (typeof WAREHOUSE_SECTORS !== 'undefined') ? (WAREHOUSE_SECTORS[currentSectorIndex] || WAREHOUSE_SECTORS[0]) : { floorColor: '#030712', gridColor: '#1e293b' };
  ctx.save();
  ctx.fillStyle = sec.floorColor || '#030712';
  ctx.fillRect(0, 0, gameWidth, gameHeight);
  ...
  ctx.scale(zoom, zoom);
  const shakeX = (typeof screenShake !== 'undefined' && screenShake > 0) ? (Math.random() * screenShake - screenShake / 2) : 0;
  const shakeY = (typeof screenShake !== 'undefined' && screenShake > 0) ? (Math.random() * screenShake - screenShake / 2) : 0;
  
  const camX = isNaN(camera.x) ? 0 : camera.x;
  const camY = isNaN(camera.y) ? 0 : camera.y;
  ctx.translate(-camX + shakeX, -camY + shakeY);

  // 1. Draw High-Contrast Epoxy WarIndustrial Polished Epoxy Warehouse Floor (No Grid)
  const floorGloss = ctx.createLinearGradient(camera.x, camera.y, camera.x + viewW, camera.y + viewH);
  floorGloss.addColorStop(0, "rgba(255, 255, 255, 0.02)");
  floorGloss.addColorStop(0.5, "rgba(255, 255, 255, 0.04)");
  floorGloss.addColorStop(1, "rgba(0, 0, 0, 0.06)");
  ctx.fillStyle = floorGloss;
  ctx.fillRect(camera.x, camera.y, viewW, viewH);

  // Stenciled Transport Boundary Markings (clean, sparse industrial walkways instead of grid)
  ctx.fillStyle = "rgba(234, 179, 8, 0.25)";
  const laneY = 960;
  if (laneY >= camera.y - 20 && laneY <= camera.y + viewH + 20) {
    ctx.fillRect(camera.x, laneY, viewW, 6);
  }
  ...
  // 3.4. Crimsonland Permanent Splatter Canvas (Optimized Sub-Region Draw)
  if (typeof splatterCanvas !== 'undefined') {
    const sx = Math.max(0, Math.min(ARENA_WIDTH - 1, camera.x - 50));
    const sy = Math.max(0, Math.min(ARENA_HEIGHT - 1, camera.y - 50));
    const sw = Math.min(viewW + 100, ARENA_WIDTH - sx);
    const sh = Math.min(viewH + 100, ARENA_HEIGHT - sy);
    if (sw > 0 && sh > 0) {
  ...
  cinemaPipeline.renderPostProcess(ctx, player.x, player.y, player.angle, screenShake, gameTime);


  // ============================================================================
  // 2.5D ISOMETRIC / AXONOMETRIC DEPTH-SORTED SCENE GRAPH (Y-SORTING)
  // ============================================================================
  const renderList = [];

  // A. 3D Warehouse Obstacles (Towering Racks, Stacked Pallets, Toi-Toi)
  for (let i = 0; i < obstacles.length; i++) {
    const o = obstacles[i];
    if (o.x + o.w > camera.x - 40 && o.x < camera.x + viewW + 40 &&
        o.y + o.h > camera.y - 40 && o.y < camera.y + viewH + 40) {
      renderList.push({
        sortY: o.y + o.h,
        fn: () => {
          if (o.type === 'rack') {
            ctx.drawImage(rackCanvas, o.x - 8, o.y - 8);
          } else if (o.type === 'pallet_stack') {
            ctx.drawImage(palletStackCanvas, o.x - 4, o.y - 4);
          } else if (o.type === 'toitoi_station') {
            // 3D Extruded ToiToi Station
            ctx.fillStyle = 'rgba(0,0,0,0.55)';
            ctx.fillRect(o.x + 8, o.y + 12, o.w + 10, o.h + 8);
            ctx.fillStyle = '#15803d';
            ctx.fillRect(o.x, o.y, o.w, o.h);
            ctx.fillStyle = '#22c55e'; // 3D Roof
            ctx.fillRect(o.x, o.y - 12, o.w, 14);
            ctx.strokeStyle = '#4ade80';
            ctx.lineWidth = 2;
            ctx.strokeRect(o.x, o.y, o.w, o.h);
            ctx.fillStyle = '#ffffff';
            ctx.font = '900 11px sans-serif';
            ctx.fillText('TOI-TOI', o.x + 6, o.y + 24);
          }
        }
      });
    }
  ...
  // B. 3D Breakable Warehouse Props
  for (let i = 0; i < mapProps.length; i++) {
    const p = mapProps[i];
    if (p.x > camera.x - 60 && p.x < camera.x + viewW + 60 &&
        p.y > camera.y - 60 && p.y < camera.y + viewH + 60) {
      renderList.push({
        sortY: p.y + p.radius,
        fn: () => drawSingleMapProp(ctx, p)
      });
    }
  }

  // C. 3D Hovering Pickups (Barcodes, Coffee, Sandwiches)
  for (let i = 0; i < dropItems.length; i++) {
    const it = dropItems[i];
    if (it.x > camera.x - 30 && it.x < camera.x + viewW + 30 &&
        it.y > camera.y - 30 && it.y < camera.y + viewH + 30) {
      renderList.push({
        sortY: it.y,
        fn: () => drawSingleDropItem(ctx, it, fontScale)
      });
    }
  }

  // D. 2.5D Animated Enemies (Swarms, Vans, Workers, Guards)
  for (let i = 0; i < enemies.length; i++) {
    const e = enemies[i];
    if (e.x + 80 > camera.x && e.x - 80 < camera.x + viewW &&
        e.y + 80 > camera.y && e.y - 80 < camera.y + viewH) {
      renderList.push({
        sortY: e.y + (e.info.radius || 15) * 0.7,
        fn: () => drawSingleEnemy(ctx, e, viewW, viewH)
      });
    }
  }

  // E. 2.5D Toyota BT Industrial Forklift Player
  renderList.push({
    sortY: player.y + (player.isForklift ? 16 : 10),
    fn: () => draw25DPlayerForklift(ctx)
  });

  // F. Kluska Companion Dog
  ...
  // SORT BY Y: Objects furthest away drawn first, nearest objects overlap them in 3D!
  renderList.sort((a, b) => a.sortY - b.sortY);

  // RENDER COMPLETE 2.5D SCENE GRAPH
...
vigGrad.addColorStop(1, edgeColor);
  ...
  // 21. Tactical Military Radar with Real-time 360° Sweep
  const mapSize = 125;
  const padding = 14;
  const mapX = gameWidth - mapSize - padding;
  const mapY = 95;
  ...
  // Tactical Range Rings & Grid lines
  ctx.strokeStyle = 'rgba(56, 189, 248, 0.22)';
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.arc(radarCenterX, radarCenterY, mapSize * 0.22, 0, Math.PI * 2);
  ...
  // Rotating Phosphor Radar Sweep Beam
  const sweepAngle = (gameTime * 2.5) % (Math.PI * 2);
  const sweepGrad = ctx.createRadialGradient(radarCenterX, radarCenterY, 2, radarCenterX, radarCenterY, mapSize * 0.65);
  sweepGrad.addColorStop(0, 'rgba(56, 189, 248, 0.4)');
  sweepGrad.addColorStop(1, 'rgba(56, 189, 248, 0.0)');
...
function startGamePlay() {
  if (typeof Engine3D !== "undefined") {
    if (!Engine3D.active) Engine3D.init();
    if (Engine3D.active) Engine3D.syncRacks(obstacles);
  }
  document.getElementById('start-screen').style.display = 'none';
  document.getElementById('intro-screen').style.display = 'none';
